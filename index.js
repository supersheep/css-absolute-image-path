...
function CssParser(opt) {
    if(!opt.hosts){
        throw 'please specify a `hosts` array in the option,' +
...
CssParser.prototype = {
     ...
     * @param  {[type]} parsed {host,ext,name}
    ...
    connector:function (parsed){
        ...
        name = mod_path.relative(dir,name);
     ...
     */
    parse: function(csspath, csscontent) {
        csspath = mod_path.resolve(csspath);
        var self = this;
         ...
         * @type {String}
         */
        var content = csscontent ||  fs.readFileSync(csspath,"utf-8");
    ...
    calculateImagePath:function(csspath,imgpath,absolute) {
        ...
        if(!fs.existsSync(real_full_path)){
            ...
            var parsed = {
                ...
                absolute:absolute,
                ext:ext,
                md5:mod_md5(fs.readFileSync(real_full_path, { encoding: null })),
                name:name
            };
